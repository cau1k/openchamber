...
export const AgentMentionAutocomplete = React.forwardRef<AgentMentionAutocompleteHandle, AgentMentionAutocompleteProps>(({
  ...
  const renderAgent = (agent: AgentInfo, index: number) => (
    <div
      key={agent.name}
      className={cn(
        'flex items-start gap-2 px-3 py-1.5 cursor-pointer rounded-none-none typography-ui-label',
...
>
  ...
  return (
    <div
      ref={containerRef}
      className="absolute z-[100] min-w-0 w-full max-w-[360px] max-h-60 bg-background border border-border rounded-none-none shadow-none bottom-full mb-2 left-0 flex flex-col"
    >
